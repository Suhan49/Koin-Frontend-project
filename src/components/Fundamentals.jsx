    ...
    <div className="bg-white pl-4 pb-6">
         <div className="flex space-x-2">
                  ...
                  <div className="flex flex-col md:flex-row space-y-2 md:space-x-35 -mb-6">
                    <div>
                    <FundamentalsBlock 
                    name={"Bitcoin"} price={crypto ? `$${crypto.current_price.toLocaleString()}` : "..."} className={" space-x-137 md:space-x-73"}/>
                    <FundamentalsBlock 
                    name={"24h Low / 24h High"} price={
  crypto
    ? `$${crypto.low_24h.toLocaleString()} / $${crypto.high_24h.toLocaleString()}`
    : "..."
} className={"space-x-97 md:space-x-33"}/>
                    <FundamentalsBlock
  name={"7d Low / 7d High"}
  price={
    crypto?.price_change_percentage_7d_in_currency != null
      ...
      : "..."
  } className={"space-x-120 md:space-x-57"}
/>
                    <FundamentalsBlock 
                    name={"Trading Volume"} price={crypto ? `$${crypto.total_volume.toLocaleString()}` : "..."} className={"space-x-107 md:space-x-43"}/>
                    <FundamentalsBlock 
                    name={"Market Cap Rank"} price={crypto ? `#${crypto.market_cap_rank}` : "..."} className={"space-x-126 md:space-x-64"}/>
                  </div>
                  <div>
                    <FundamentalsBlock 
                    name={"Market Cap"} price={crypto ? `$${crypto.market_cap.toLocaleString()}` : "..."} className={"space-x-109 md:space-x-45"}/>
                    <FundamentalsBlock 
  ...
  crypto
    ...
    : "..."
} className={"space-x-113 md:space-x-49"}/>
                    <div className="mt-2 " >
        <span className="opacity-60 ">All-Time High</span>
        <span className=" pl-108 md:pl-44">{crypto ? `$${crypto.ath.toLocaleString()}` : "..."} </span>
        <span className="text-red-700 ">{crypto ? `${crypto.ath_change_percentage.toFixed(2)}%` : "..."}</span>
    </div>
    <div className="text-xs pl-148 md:pl-83 opacity-75">{crypto
  ...
  : "..."}</div>
    <div className="w-165 md:w-100 h-0.5 opacity-55  -mx-1 bg-gray-400 rounded-sm"></div>
        ...
        <span className="pl-107 md:pl-43">{crypto ? `$${crypto.atl.toLocaleString()}` : "..."}</span>
